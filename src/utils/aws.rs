...
use aws_sdk_s3::{config::Builder, operation::get_object::{GetObjectError, GetObjectOutput}, Client};
...
pub async fn list_keys_to_map(client: Client, bucket: &str, prefix: &str) -> Result<HashMap<String, (Option<i64>, Option<i64>)>> {
	let mut stream = client
        .list_objects_v2()
        .bucket(bucket)
        .prefix(prefix)
	...
	let mut files: HashMap<String, (Option<i64>, Option<i64>)> = HashMap::new();
    while let Some(objects) = stream.next().await.transpose()? {
        for obj in objects.contents() {
            if let Some(key) = obj.key() {
                if !key.ends_with('/') {
                    ...
                    let dt= match obj.last_modified() {
                        Some(dt) => {
                            let dt = dt.secs();
                            Some(dt)
                        },
                    ...
                    files.insert(file_name, (file_size, dt));
                }
            }
        }
    }
